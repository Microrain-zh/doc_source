...
//static struct gpio_desc *ds18b20_test_pin;
...
{
	int timeout_count = 1000000;
	unsigned int val;

	//gpiod_set_value(ds18b20_test_pin, 0);
	...
	//gpiod_set_value(ds18b20_test_pin, 1);
	//printk("gpiod_get_value(ds18b20_data_pin) = %d\n", gpiod_get_value(ds18b20_data_pin));

	if (!timeout_count)
	{
		printk("%s %s line %d, count %d\n", __FILE__, __FUNCTION__, __LINE__, 1000000 - timeout_count);
		return -1;
	}
...
static int ds18b20_probe(struct platform_device *pdev)
{
	...
	//ds18b20_test_pin = gpiod_get(&pdev->dev, "test", GPIOD_OUT_HIGH);
